...
def evaluate(
    ...
    if np.std(uncs) < 1e-8:
        corr = float("nan")
        print("[test] Error–Uncertainty correlation: nan (no uncertainty in deterministic mode)")
    else:
...
def main() -> None:
    ...
    ece, mean_sigma = plot_calibration(models, val_loader, device, K=K, q_conformal=q_conformal)
    ...
    print(f"[test]   mean σ (sharpness): {mean_sigma:.4f}")
    print(f"[test]   Winkler Score (90%)      : {metrics['winkler']:.6f}")
